...
def check_answers_integrated_model(model, demo_data, eval_data, device="cuda"):
    contexts = [get_context(demo_data, (eval_data[0][idx:idx+1], eval_data[1][idx:idx+1])) for idx in range(len(eval_data[0]))]
    output = model.generate(contexts, do_sample=False, max_new_tokens=3)
    # check if the answer is corret
    answers = [output[idx][len(contexts[idx]):] for idx in range(len(contexts))]
    num_correct = 0
...
def remove_and_get_correct_num(demo_data, test_data, remove_idx, model, tokenizer=None, batch_size=4, is_corrupt=False, device="cuda"):
    ...
    for idx in range(len(test_data[0]) // batch_size):
        ...
        if tokenizer is None:
            num_correct += check_answers_integrated_model(model, new_demo_data, cur_test_data, device=device)
        else:
...
def compute_vinay_infl(demo_data, eval_data, model, tokenizer):
    ...
    with torch.no_grad():
        for demo in demos:
            ...
            label_pos = get_label_pos(tokenizer, input_ids[0], list(map_abcd_to_int.keys()), model_name=model_name)[:-1]
    ...
    for idx in range(len(eval_data[0])):
        cur_infls = calc_influence_single(linear_model, train_data_loader, eval_data_loader, idx, 0, len(demo_data[0]) // 4, 4)
        for i in range(len(demo_data[0])):
...
def compute_datamodel(demo_data, eval_data, model, tokenizer):
    def get_datamodel_f(context, label_id, num_classes):
        input_ids = tokenizer(context).input_ids
        input_ids = torch.tensor(input_ids).unsqueeze(0).to(device)
        out = model(input_ids, output_hidden_states=False, output_attentions=False)

        prefix = "▁"

        # find the token id for A and B
        a_id = tokenizer.convert_tokens_to_ids(f"{prefix}A")
        b_id = tokenizer.convert_tokens_to_ids(f"{prefix}B")
        c_id = tokenizer.convert_tokens_to_ids(f"{prefix}C")
        d_id = tokenizer.convert_tokens_to_ids(f"{prefix}D")
        e_id = tokenizer.convert_tokens_to_ids(f"{prefix}E")

        labels = np.asarray([a_id, b_id, c_id, d_id, e_id])

        other_classes = np.asarray([x for x in range(num_classes) if x != label_id])

        all_logits = out.logits[0, -1, labels[other_classes]].max()
        correct_logits = out.logits[0, -1, labels[label_id]]

        return 2.0 * correct_logits - all_logits

    def get_indicator(example_ids, order):
        assert len(example_ids) == len(order)
        k = len(example_ids)
        return example_ids * k + order
    ...
    indicator_len = len(demo_data[0]) * K + 1
    datamodel = torch.nn.Embedding(indicator_len, 1).to(device)
    criterion = torch.nn.MSELoss()
    optimizer = torch.optim.Adam(datamodel.parameters(), lr=0.01)
    ...
    for idx in range(len(eval_data[0])):
        ...
        for _ in range(M):
            select_idx = np.random.choice(len(demo_data[0]), K, replace=False)

            for perm in all_perms:
                perm = list(perm)
                optimizer.zero_grad()
                example_ids = np.zeros(len(perm), dtype=np.int32)
                for i, p in enumerate(perm):
                    example_ids[p] = select_idx[i]
                cur_demo_data = ([demo_data[0][i] for i in example_ids], [demo_data[1][i] for i in example_ids])
                indicator = get_indicator(select_idx, perm)
                # append indicator_len-1 at the back of indicator
                indicator = np.append(indicator, indicator_len-1)
                cur_context = get_context(cur_demo_data, cur_eval_data)
                f = get_datamodel_f(cur_context, map_abcd_to_int[cur_demo_data[1][-1]], num_classes)
                indicator_input = torch.tensor(indicator).to(device)
                out = datamodel(indicator_input).sum()
                loss = criterion(out, f)
                loss.backward()
                optimizer.step()  

        for train_id in range(len(demo_data[0])):
            for pos in range(K):
                id_ = train_id * K + pos
                w = list(datamodel.parameters())[0][id_, 0]
                if w > 0:
                    infls[train_id] += 1
...
def run(num_remove):
    ...
    elif method_name in ["influence", "nguyen_infl", "vinay_infl", "datamodel", "random"]:
    ...
    for seed in range(num_trials):
        ...
        for demo_data, eval_data in dataset:
            ...
            if method_name == "influence":
                ...
                num_success_rem_low += remove_and_get_correct_num(demo_data, test_data, remove_idx, model, tokenizer, batch_size=batch_size, is_corrupt=corrupt, device=device)
                ...
                remove_idx = np.argsort(attrs)[:num_remove]
                num_success_rem_low += remove_and_get_correct_num(demo_data, test_data, remove_idx, model, tokenizer, batch_size=batch_size, is_corrupt=corrupt, device=device)
            elif method_name == "datamodel":
                start = time.time()
                attrs = compute_datamodel(demo_data, eval_data, model, tokenizer)
                end = time.time()
                remove_idx = np.argsort(attrs)[:num_remove]
                num_success_rem_low += remove_and_get_correct_num(demo_data, test_data, remove_idx, model, tokenizer, batch_size=batch_size, is_corrupt=corrupt, device=device)
...
if __name__ == "__main__":
    ...
    assert method_name in ["influence", "ig", "sig", "attention", "lime", "reagent", "nguyen_infl", "vinay_infl", "datamodel", "random"], f"Method {method_name} not implemented"
    ...
    for num_remove in num_removes:
        acc_rem_low, wall_time = run(num_remove)
