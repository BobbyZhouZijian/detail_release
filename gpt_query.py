...
async def palm_generate(func, model, prompt, temperature, max_output_tokens):
    async with func(model=model, prompt=prompt, temperature=temperature,max_output_tokens=max_output_tokens) as resp:
        return await resp
...
class GPT_Forward(LLM):
    ...
    def __init__(self, config, needs_confirmation=False, disable_tqdm=False):
    ...
    def generate_text(self, prompt, n, temperature=0, use_seed=True):
        ...
        for prompt_batch in tqdm(prompt_batches, disable=self.disable_tqdm):
            ...
            text += self.__async_generate(prompt_batch, n, temperature, use_seed=use_seed)
        return text
    ...
    def __async_generate(self, prompt, n, temperature=0, use_seed=True):
